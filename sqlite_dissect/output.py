...
from sqlite_dissect.utilities import decode_str, has_content
    ...
    for pointer_map_page in version.pointer_map_pages:
        ...
        if not pointer_map_page.pointer_map_entries:
            ...
            log_message = log_message.format(
                ...
                pointer_map_page.page_version_number,
                version.number,
            )
        ...
        for entry in pointer_map_page.pointer_map_entries:
            if hexlify(str.encode(last_type_seen)) != hexlify(
                ...
                breakdown = (
                    ...
                    entry.page_number - 1,
                    pages,
                    hexlify(str.encode(last_entry.page_type)),
                )
        ...
        if version.master_schema_modified:
            ...
            for (
                ...
                if md5_hash_identifier not in master_schema_entries:
                    ...
                    added_string = added_string.format(
                        version_number,
                        master_schema_entry.root_page_number,
                        master_schema_entry.row_type,
                        master_schema_entry.name,
                    ...
                    updated_string = updated_string.format(
                        version_number,
                        previous_root_page_number,
                        master_schema_entry.root_page_number,
                        master_schema_entry.row_type,
                        master_schema_entry.name,
                        master_schema_entry.table_name,
            ...
            for (
                ...
                if md5_hash_identifier not in modified_master_schema_entries:
                    ...
                    removed_string = removed_string.format(
                        version_number,
                        master_schema_entry.root_page_number,
                        master_schema_entry.row_type,
                        master_schema_entry.name,
    ...
    if version.pointer_map_pages:
        ...
        for pointer_map_entry_breakdown in get_pointer_map_entries_breakdown(version):
            ...
            string = string.format(
                ...
                pointer_map_entry_breakdown[2],
                pointer_map_entry_breakdown[3],
        ...
        if len(unallocated_content):
            if (
                ...
                string = string.format(
                    ...
                    page.unallocated_space_end_offset,
                    page.unallocated_space_length,
            ...
            for freeblock in page.freeblocks:
                ...
                if len(freeblock_content) and has_content(freeblock_content):
                    ...
                    string = string.format(
                        ...
                        freeblock.end_offset,
                        freeblock.content_length,
