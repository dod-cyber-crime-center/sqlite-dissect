...
def test_record_init(
    ...
    if expected_value == RECORD_ERROR:
        with pytest.raises(RecordParsingError):
            _ = Record(
                page, payload_offset, payload_byte_size, bytes_on_first_page, overflow
            )
        ...
        assert (
            record.end_offset
            == payload_offset + bytes_on_first_page
            == record.body_end_offset
        )
        ...
        assert (
            record.header_byte_size_varint_length
            == decode_varint(page, payload_offset)[1]
        )
        assert record.header_start_offset == payload_offset
        assert (
            record.header_end_offset
            == payload_offset + decode_varint(page, payload_offset)[0]
            == record.body_start_offset
        )

        total_record_content = (
            page[payload_offset : payload_offset + bytes_on_first_page] + overflow
        )
