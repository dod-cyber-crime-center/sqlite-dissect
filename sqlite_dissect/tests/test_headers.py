...
from sqlite_dissect.file.wal.header import WriteAheadLogFrameHeader, WriteAheadLogHeader
from sqlite_dissect.file.wal_index.header import (
    WriteAheadLogIndexCheckpointInfo,
    WriteAheadLogIndexHeader,
    WriteAheadLogIndexSubHeader,
)
...
def test_db_header_init(permuted_database):
    if permuted_database["expected_result"] == VALUE_ERROR:
        ...
        assert (
            db_header.page_size == MAXIMUM_PAGE_SIZE
            if unpack(b">H", permuted_database["byte_array"][16:18])[0]
            == MAXIMUM_PAGE_SIZE_INDICATOR
            else unpack(b">H", permuted_database["byte_array"][16:18])[0]
        )
        assert db_header.file_format_write_version == ord(
            permuted_database["byte_array"][18:19]
        )
        assert db_header.file_format_read_version == ord(
            permuted_database["byte_array"][19:20]
        )
        assert db_header.reserved_bytes_per_page == ord(
            permuted_database["byte_array"][20:21]
        )
        assert db_header.maximum_embedded_payload_fraction == ord(
            permuted_database["byte_array"][21:22]
        )
        assert db_header.minimum_embedded_payload_fraction == ord(
            permuted_database["byte_array"][22:23]
        )
        assert db_header.leaf_payload_fraction == ord(
            permuted_database["byte_array"][23:24]
        )
        assert (
            db_header.file_change_counter
            == unpack(b">I", permuted_database["byte_array"][24:28])[0]
        )
        assert (
            db_header.database_size_in_pages
            == unpack(b">I", permuted_database["byte_array"][28:32])[0]
        )
        assert (
            db_header.first_freelist_trunk_page_number
            == unpack(b">I", permuted_database["byte_array"][32:36])[0]
        )
        assert (
            db_header.number_of_freelist_pages
            == unpack(b">I", permuted_database["byte_array"][36:40])[0]
        )
        assert (
            db_header.schema_cookie
            == unpack(b">I", permuted_database["byte_array"][40:44])[0]
        )
        assert (
            db_header.schema_format_number
            == unpack(b">I", permuted_database["byte_array"][44:48])[0]
        )
        assert (
            db_header.default_page_cache_size
            == unpack(b">I", permuted_database["byte_array"][48:52])[0]
        )
        assert (
            db_header.largest_root_b_tree_page_number
            == unpack(b">I", permuted_database["byte_array"][52:56])[0]
        )
        assert (
            db_header.database_text_encoding
            == unpack(b">I", permuted_database["byte_array"][56:60])[0]
        )
        assert (
            db_header.user_version
            == unpack(b">I", permuted_database["byte_array"][60:64])[0]
        )
        assert (
            db_header.incremental_vacuum_mode
            == unpack(b">I", permuted_database["byte_array"][64:68])[0]
        )
        assert (
            db_header.application_id
            == unpack(b">I", permuted_database["byte_array"][68:72])[0]
        )
        assert (
            db_header.reserved_for_expansion == permuted_database["byte_array"][72:92]
        )
        assert (
            db_header.version_valid_for_number
            == unpack(b">I", permuted_database["byte_array"][92:96])[0]
        )
        assert (
            db_header.sqlite_version_number
            == unpack(b">I", permuted_database["byte_array"][96:100])[0]
        )
        assert (
            db_header.md5_hex_digest
            == md5(permuted_database["byte_array"]).hexdigest().upper()
        )
...
@pytest.mark.parametrize(
    "byte_array, expected_value", test_write_ahead_log_header_params
)
def test_write_ahead_log_header_init(byte_array, expected_value):
    if expected_value == HEADER_ERROR:
        ...
        assert (
            wal_header.checkpoint_sequence_number == unpack(b">I", byte_array[12:16])[0]
        )
...
@pytest.mark.parametrize(
    "wal_index_header_byte_array, expected_value", test_wal_index_header_params
)
def test_wal_index_header_init(wal_index_header_byte_array, expected_value):
    if expected_value == VALUE_ERROR:
        ...
        assert (
            wal_index_header.page_size
            == unpack(b"<H", wal_index_header_byte_array[14:16])[0]
        )
        assert wal_index_header.endianness == ENDIANNESS.LITTLE_ENDIAN
        assert (
            wal_index_header.lock_reserved
            == wal_index_header_byte_array[
                lock_reserved_start_offset:lock_reserved_end_offset
            ]
        )
        assert wal_index_header.md5_hex_digest == get_md5_hash(
            wal_index_header_byte_array
        )


with open(os.path.join(DB_FILES, "chinook.sqlite-shm"), "rb") as wal_index_file:
    test_wal_index_sub_header_byte_array = wal_index_file.read(
        WAL_INDEX_SUB_HEADER_LENGTH
    )
...
test_wal_index_sub_header_params = [
    ...
    (
        replace_bytes(test_wal_index_sub_header_byte_array, b"\x00\x00\x00\x00", 0),
        0,
        HEADER_ERROR,
    ),
    (
        replace_bytes(test_wal_index_sub_header_byte_array, b"\x00\x2D\xE2\x18", 0),
        0,
...
@pytest.mark.parametrize(
    "wal_index_sub_header_byte_array, index, expected_value",
    test_wal_index_sub_header_params,
)
def test_wal_index_sub_header_init(
    wal_index_sub_header_byte_array, index, expected_value
):
    if expected_value == VALUE_ERROR:
        with pytest.raises(ValueError):
        ...
        wal_index_sub_header = WriteAheadLogIndexSubHeader(
            index, wal_index_sub_header_byte_array
        )
        ...
        assert (
            ...
            == unpack(b"<I", wal_index_sub_header_byte_array[0:4])[0]
        )
        assert (
            wal_index_sub_header.unused_padding_field
            == unpack(b"<I", wal_index_sub_header_byte_array[4:8])[0]
        )
        assert (
            wal_index_sub_header.change_counter
            == unpack(b"<I", wal_index_sub_header_byte_array[8:12])[0]
        )
        assert wal_index_sub_header.initialized == ord(
            wal_index_sub_header_byte_array[12:13]
        )
        assert wal_index_sub_header.checksums_in_big_endian == ord(
            wal_index_sub_header_byte_array[13:14]
        )
        assert (
            wal_index_sub_header.page_size
            == unpack(b"<H", wal_index_sub_header_byte_array[14:16])[0]
        )
        assert (
            wal_index_sub_header.last_valid_frame_index
            == unpack(b"<I", wal_index_sub_header_byte_array[16:20])[0]
        )
        assert (
            wal_index_sub_header.database_size_in_pages
            == unpack(b"<I", wal_index_sub_header_byte_array[20:24])[0]
        )
        assert (
            wal_index_sub_header.frame_checksum_1
            == unpack(b"<I", wal_index_sub_header_byte_array[24:28])[0]
        )
        assert (
            wal_index_sub_header.frame_checksum_2
            == unpack(b"<I", wal_index_sub_header_byte_array[28:32])[0]
        )
        assert (
            wal_index_sub_header.salt_1
            == unpack(b"<I", wal_index_sub_header_byte_array[32:36])[0]
        )
        assert (
            wal_index_sub_header.salt_2
            == unpack(b"<I", wal_index_sub_header_byte_array[36:40])[0]
        )
        assert (
            wal_index_sub_header.checksum_1
            == unpack(b"<I", wal_index_sub_header_byte_array[40:44])[0]
        )
        assert (
            wal_index_sub_header.checksum_2
            == unpack(b"<I", wal_index_sub_header_byte_array[44:48])[0]
        )

        # TODO: Replace utilities get_md5_hash with independent implementation
        assert wal_index_sub_header.md5_hex_digest == get_md5_hash(
            wal_index_sub_header_byte_array
        )


checkpoint_info_start_offset = (
    WAL_INDEX_NUMBER_OF_SUB_HEADERS * WAL_INDEX_SUB_HEADER_LENGTH
)
checkpoint_info_end_offset = (
    checkpoint_info_start_offset + WAL_INDEX_CHECKPOINT_INFO_LENGTH
)
test_wal_index_checkpoint_info_byte_array = wal_index_byte_array[
    checkpoint_info_start_offset:checkpoint_info_end_offset
]

test_wal_index_checkpoint_info_params = [
    (test_wal_index_checkpoint_info_byte_array, ENDIANNESS.LITTLE_ENDIAN, SUCCESS),
    (
        test_wal_index_checkpoint_info_byte_array[:10],
...
@pytest.mark.parametrize(
    "wal_index_checkpoint_info_byte_array, endianness, expected_value",
    test_wal_index_checkpoint_info_params,
)
def test_wal_index_checkpoint_info_init(
    wal_index_checkpoint_info_byte_array, endianness, expected_value
):
    if expected_value == VALUE_ERROR:
        with pytest.raises(ValueError):
            _ = WriteAheadLogIndexCheckpointInfo(
                wal_index_checkpoint_info_byte_array, endianness
            )

    else:
        wal_index_checkpoint_info = WriteAheadLogIndexCheckpointInfo(
            wal_index_checkpoint_info_byte_array, endianness
        )
        ...
        assert (
            ...
            == unpack(b"<I", wal_index_checkpoint_info_byte_array[0:4])[0]
        )
        ...
        for index in range(WAL_INDEX_READER_MARK_SIZE):
            ...
            assert (
                wal_index_checkpoint_info.reader_marks[index]
                == unpack(
                    b"<I", wal_index_checkpoint_info_byte_array[start_offset:end_offset]
                )[0]
            )

        assert wal_index_checkpoint_info.md5_hex_digest == get_md5_hash(
            wal_index_checkpoint_info_byte_array
        )
...
@pytest.mark.parametrize(
    "rollback_journal_header_byte_array, expected_value", test_journal_header_params
)
def test_journal_header_init(rollback_journal_header_byte_array, expected_value):
    if expected_value == VALUE_ERROR:
        ...
        assert journal_header.page_count == (
            ...
            if rollback_journal_header_byte_array[8:12]
            == ROLLBACK_JOURNAL_HEADER_ALL_CONTENT
            else unpack(b">I", rollback_journal_header_byte_array[8:12])[0]
        )

        assert (
            journal_header.random_nonce_for_checksum
            == unpack(b">I", rollback_journal_header_byte_array[12:16])[0]
        )
        assert (
            journal_header.initial_size_of_database_in_pages
            == unpack(b">I", rollback_journal_header_byte_array[16:20])[0]
        )
        assert (
            journal_header.disk_sector_size
            == unpack(b">I", rollback_journal_header_byte_array[20:24])[0]
        )
        assert (
            journal_header.page_size
            == journal_header.size_of_pages_in_journal
            == unpack(b">I", rollback_journal_header_byte_array[24:28])[0]
        )
        assert journal_header.md5_hex_digest == get_md5_hash(
            rollback_journal_header_byte_array
        )
