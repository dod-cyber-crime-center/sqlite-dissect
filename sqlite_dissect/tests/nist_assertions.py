...
def assert_md5_equals(initial_hash, final_hash, filename):
    assert initial_hash == final_hash, (
        "MD5 Hash value of file %s does not match. File has been altered!\n"
...
def assert_correct_page_size(reported_size, correct_size):
    assert reported_size == correct_size, (
        "The program reports an incorrect page size!\n"
...
def assert_correct_journal_mode(reported_mode, correct_mode, version):
    version_dict = {"r": "read version", "w": "write version"}
    ...
    assert reported_mode.upper() == journal_dict[correct_mode.upper()], (
        "The program reports an incorrect journal mode (%s)!\nCorrect mode: %s\n"
...
def assert_correct_encoding(reported_enc, correct_enc):
    assert reported_enc.upper() == correct_enc.upper(), (
        "The program reports and incorrect database text encoding!\n"
        "Correct encoding: %s\nReported encoding: %s" % (correct_enc, reported_enc)
    )
...
def assert_correct_key(reported_key, correct_key, table):
    assert reported_key == correct_key, (
        "The program reports an incorrect primary key for table %s!\nCorrect key: %s\n"
...
def assert_correct_recovery_cause(reported_cause, correct_cause, row_key, table):
    cause_dict = {"d": "A deletion", "u": "An update"}
...
def assert_correct_table_name(reported_name, correct_name, elmnt):
    assert reported_name == correct_name, (
        "The program reports an incorrect table for data element %s!\n"
...
def assert_correct_transactions(reported_transactions, correct_transactions, wal):
    ...
    assert extra_transactions, (
        "The program reports more transactions than expected in WAL file %s!\n"
