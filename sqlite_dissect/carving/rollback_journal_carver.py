...
from sqlite_dissect.constants import FILE_TYPE, LOGGER_NAME, PAGE_TYPE
        ...
        while has_data:
            ...
            page_checksum = hexlify(
                rollback_journal.file_handle.read_data(
                    offset + page_record_header_size + page_size,
                    page_record_checksum_size,
                )
            )

            logger.debug(
                "At offset: %s page Number: %s of type: %s has content with checksum of: %s"
