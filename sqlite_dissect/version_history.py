...
from sqlite_dissect.constants import (
    BASE_VERSION_NUMBER,
    CELL_SOURCE,
    COMMIT_RECORD_BASE_VERSION_NUMBER,
    LOGGER_NAME,
...
from sqlite_dissect.exception import (
    VersionParsingError,
    WalCommitRecordParsingError,
    WalFrameParsingError,
)
    ...
    def __init__(self, database, write_ahead_log=None):
        ...
        if self._write_ahead_log:
                ...
                if frame.commit_frame:
                    ...
                    if commit_record.database_header_modified:
                        ...
                        if not last_database_header:
                            log_message = (
                                "Database header was detected as modified for commit record version: {} "
                    ...
                    if commit_record.master_schema_modified:
                        ...
                        if not last_master_schema:
                            log_message = (
                                "Master schema was detected as modified for commit record version: {} "
            ...
            if len(frames) > 0:
                ...
                log_message = (
                    "Version (commit record): {} has additional frames beyond the last commit frame found "
...
class VersionHistoryParser(VersionParser):
    def __init__(
        ...
        log_message = (
            "Creating version history parser for master schema entry with name: {} table name: {} "
        ...
        if signature:
            ...
            if signature.row_type != MASTER_SCHEMA_ROW_TYPE.TABLE:
                log_message = (
                    "Not carving version history parser for master schema entry with name: {} table "
        ...
        if self.carve_freelist_pages and not self.signature:
            log_message = (
                "Carve freelist pages set with no signature defined.  A signatures is needed in order to "
    ...
    def __iter__(self):
        if self.row_type not in [
            ...
            log_message = (
                "Virtual table found with root page 0 for in version history parser for master schema "
    ...
    class VersionParserIterator:
        ...
        def __init__(
            ...
            root_page_number_version_index,
            signature=None,
            carve_freelist_pages=False,
        ):
            self._name = name
        ...
        def stringify(self, padding="", print_cells=True):
            string = (
                ...
                + "Page Type: {}\n"
                + padding
                + "Parser Starting Version Number: {}\n"
                + padding
                + "Parser Ending Version Number: {}\n"
                + padding
                + "Root Page Number Version Index: {}\n"
                + padding
                + "Current Version Number: {}\n"
                + padding
                + "Current B-Tree Page Numbers: {}\n"
                + padding
                + "Carve Freelist Pages: {}"
            )
        ...
        def next(self):
            ...
            if self._current_version_number <= self._parser_ending_version_number:
                ...
                commit = Commit(
                    ...
                    version.file_type,
                    self._current_version_number,
                    version.database_text_encoding,
                    self._page_type,
                    root_page_number,
                    self._current_b_tree_page_numbers,
                )
                ...
                if b_tree_updated:
                    ...
                    if self._page_type == PAGE_TYPE.B_TREE_TABLE_LEAF:
                        ...
                        if self._signature:
                            log_message = (
                                "Carving table master schema entry name: {} for page type: {} for version: "
                        ...
                        log_message = (
                            "Invalid page type: {} found for version: {} iterating between versions {} "
                            "and {} over b-tree page numbers: {} with updated b-tree pages: {}."
                        )
                        log_message = log_message.format(
                            self._page_type,
                        ...
                        if self._page_type == PAGE_TYPE.B_TREE_TABLE_LEAF:
                            ...
                            for (
                                ...
                                carvings = SignatureCarver.carve_unallocated_space(
                                    ...
                                    freelist_page_number,
                                    freelist_page.unallocated_space_start_offset,
...
class Commit:
    def __init__(
        ...
        root_page_number,
        b_tree_page_numbers,
        updated_b_tree_page_numbers=None,
        freelist_pages_carved=False,
    ...
    def stringify(self, padding="", print_cells=True):
        string = (
            ...
            + padding
            + "Freelist Pages Carved: {}\n"
            + padding
            + "Updated Freelist Page Numbers: {}\n"
        )
        string = string.format(
            self.version_number,
