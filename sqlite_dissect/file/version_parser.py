...
class VersionParser:
    ...
    def __init__(
        ...
        if version_number is None and ending_version_number:
            log_message = (
                "Version number not specified where ending version number was specified as: {} for "
        ...
        if version_number is not None and version_number == ending_version_number:
            log_message = (
                "Version number: {} specified where ending version number was also specified as: {} for "
        ...
        if master_schema_entry.row_type not in [
            MASTER_SCHEMA_ROW_TYPE.TABLE,
            MASTER_SCHEMA_ROW_TYPE.INDEX,
        ]:
            log_message = (
                "Invalid master schema entry row type: {} for master schema entry with root page "
        ...
        if isinstance(master_schema_entry, VirtualTableRow):
            log_message = (
                "A virtual table row type was found for the version parser which is not fully supported "
        ...
        for version_number in range(
            ...
            if not master_schema:
                log_message = (
                    "Master schema was unable to be found in starting version number: {} while parsing "
            ...
            if self.master_schema_entry_md5_hash_identifier in entries_dictionary:
                ...
                if ending_version_number is None:
                    ...
                    if self.root_page_number_version_index:
                        log_message = (
                            "The root page number version index has already been populated with values "
                    ...
                    if not self.root_page_number_version_index:
                        log_message = (
                            "The root page number version index has not already been populated with values "
                    ...
                    log_message = (
                        "Version number: {} did not have a master schema entry for the previous "
            ...
            if starting_version_number is None and ending_version_number is None:
                log_message = (
                    "Was unable to find any matching schema entries between version numbers {} "
    ...
    def stringify(self, padding=""):
        string = (
            ...
            + padding
            + "Master Schema Entry MD5 Hash Identifier: {}\n"
            + padding
            + "Version Number: {}\n"
            + padding
