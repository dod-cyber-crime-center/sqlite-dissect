...
from sqlite_dissect.constants import (
    ...
    ROLLBACK_JOURNAL_HEADER_HEX_STRING,
...
class RollbackJournalHeader(SQLiteHeader):
    ...
    def stringify(self, padding=""):
        string = (
            ...
            + padding
            + "Initial Size of Database in Pages: {}\n"
            + padding
            + "Disk Sector Size: {}\n"
            + padding
            + "Size of Pages in Journal: {}\n"
            + padding
            + "MD5 Hex Digest: {}"
        )
        return string.format(
            hexlify(self.header_string),
            self.page_count,
            self.random_nonce_for_checksum,
            self.initial_size_of_database_in_pages,
            self.disk_sector_size,
            self.size_of_pages_in_journal,
            self.md5_hex_digest,
        )


class RollbackJournalPageRecordHeader:
    def __repr__(self):
        return self.__str__()
